...
negex_en = open(os.path.join("data/vocab_list/", "negex_en.txt"), "r")
negex_fr = open(os.path.join("data/vocab_list/", "negex_fr.txt"), "r")
NEGEX_LIST_EN = [line.strip("\n") for line in negex_en.readlines()]
NEGEX_LIST_FR = [line.strip("\n") for line in negex_fr.readlines()]

negex_sent_en = open(os.path.join("data/vocab_list/", "negex_sep_en.txt"), "r")
negex_sent_fr = open(os.path.join("data/vocab_list/", "negex_sep_fr.txt"), "r")
...
class TextReport:
    ...
    def __init__(self, file_obj, lang):
        ...
        self.ontology_path = os.path.join(
            current_app.config["ONTOLOGY_FOLDER"], "ontology_default.json"
        )
        self.raw_text = ""
        self.text_as_list = []
        ...
        if self.lang == "fra":
            ...
            self.negexlist = NEGEX_LIST_FR
            self.negex_sent = NEGEX_SENT_FR
        if self.lang == "eng":
            self.nlp = spacy.load("en_core_web_sm")
            self.negexlist = NEGEX_LIST_EN
            self.negex_sent = NEGEX_SENT_EN
        self.all_stopwords = self.nlp.Defaults.stop_words
        self.results_match_dict = {}
        self.image_stack = convert_from_bytes(self.file_obj)
    ...
    def _match_ngram_ontology(self, full_ngrams) -> list:
        ...
        ontology_terms = []
        match_list = []
        json_onto = json.load(open("ontology.json", "rb"))
        for i in json_onto:
